...
if ngpus > 1:
    pretrained_device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
else:
...
def main():
    ...
    if args.pretrained:
        ...
        model_ffn.load_state_dict(state_dict)
    ...
    for layer_idx in range(len(model_pretrained.features)):
        ...
        if isinstance(module, nn.Conv2d): 
            print('processing conv #', conv_idx)
            if conv_idx > 0:
                # layers
                conv = module
                ...
                # save
                with open(args.save_path+'/conv'+ str(conv_idx+1) + '_' + str(kmax) + '_rr_e30.pkl', 'wb') as f:
                    pickle.dump({'D': D, 'U': U, 'V': V}, f, pickle.HIGHEST_PROTOCOL)
    ...
    validate(val_loader, model_ffn, criterion, args)
    save_state_dict(model_ffn.state_dict(), args.save_path, args.arch+'_conv_e30.pth')
    ...
    for layer_idx in range(len(model_pretrained.classifier)):
        ...
        if isinstance(module, nn.Linear): 
            print('processing FC'+str(6+fc_idx))
            # layers
            conv = module
            # weights and bias
            W = conv.weight.data#.cpu()
            ...
            print('run sdd')
            start_time = time.time()
            D, U, V = sdd(W.cpu().numpy(), kmax, max_epoch=1)
            end_time = time.time()
    ...
    validate(val_loader, model_ffn, criterion, args)
    save_state_dict(model_ffn.state_dict(), args.save_path, 'alexnet_bn_conv_fc.pth')
    upbn(train_loader, model_ffn, criterion, optimizer, args, 1000)
    validate(val_loader, model_ffn, criterion, args)
    save_state_dict(model_ffn.state_dict(), args.save_path, 'alexnet_bn_conv_fc_upbn.pth')
